...
temp = new_transactions.groupby('card_id')['purchase_amount'].sum().to_frame('total_new_purchase_amount')
temp = np.exp(temp.total_new_purchase_amount)/(1+np.exp(temp.total_new_purchase_amount))
temp = temp.to_frame('total_new_purchase_amount')
df = df.merge(temp,how='left',left_index=True, right_index=True)
df.total_new_purchase_amount.fillna(0,inplace=True)

# target = df.total_new_purchase_amount
target = df.target
...
feats = [f for f in df2.columns if f not in FEATS_EXCLUDED]
feats = [c for c in feats if c in keeping]

from sklearn.model_selection import train_test_split
train_df = df2.replace([np.inf, -np.inf], np.nan)
...
for n_fold, (train_idx, valid_idx) in enumerate(kf.split(train_df[feats])):
        ...
        train_x, train_y = new_train[feats].iloc[train_idx], target.values[train_idx]
        valid_x, valid_y = new_train[feats].iloc[valid_idx], target.values[valid_idx]

        filepath="temp_best.hdf5"
        checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
        callbacks_list = [checkpoint]
        model = DNN()
        model.fit(train_x,train_y,validation_data=(valid_x,valid_y),batch_size=512,epochs=10,verbose=1,callbacks=callbacks_list)
